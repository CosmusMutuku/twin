...
# Bedrock model or inference profile ARN
# Supports both:
# - Model IDs (e.g., "amazon.nova-micro-v1:0")
...
# Bedrock call logic (supports modelId or inferenceProfileArn)
def call_bedrock(conversation: List[Dict], user_message: str) -> str:
    """Call AWS Bedrock with conversation history"""
    ...
    # System prompt (Bedrock convention: send as role=user)
    messages.append({
        "role": "user",
        "content": [{"text": f"System: {prompt()}"}]
    })
    ...
    try:
        # Check if the provided ID is an ARN
