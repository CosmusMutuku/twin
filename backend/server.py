...
# Initialize Bedrock client (cross-region safe)
bedrock_client = boto3.client(
    service_name="bedrock-runtime",
    region_name=os.getenv("DEFAULT_AWS_REGION", "us-east-1")  # Bedrock model region
)
...
# Call Bedrock safely (cross-region)
def call_bedrock(conversation: List[Dict], user_message: str) -> str:
    """Call AWS Bedrock with conversation history"""
    ...
    try:
        ...
        assistant_response = response['body'].read().decode('utf-8')
